...
Ajam = 1.0  # strength of jammer relative to desired
station_id = 0  # "callsign" of radar. Totally uncorrelated with other callsigns
dist_m = (
    10e3
)  # note too long delays will just wrap with FFT-based delay. Would have to do integer samples + FFT-shift.
c = 299792458  # [m/s]
#%%
if __name__ == "__main__":
    ...
    p = ArgumentParser(
        description="generate PSK waveforms, and optionally transmit them via raspberry pi"
    )
    p.add_argument("-c", "--codelen", type=int, default=10000, help="Code length ")
    p.add_argument(
        "--filter",
        help="smooth transmit waveform to limit needless bandwidth",
        action="store_true",
    )
    ...
    tx = waveform_to_file(
        station_id, p.codelen, filt=p.filter, outpath=p.outpath, verbose=p.verbose
    )
    ...
    if p.freqmhz:  # on-air transmission
        ...
        rx = sim_iono(
            tx, p.fs, dist_m, p.codelen, Nstd, Ajam, station_id, p.filter, p.outpath, p.verbose
        )
